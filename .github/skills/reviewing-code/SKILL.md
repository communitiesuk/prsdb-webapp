...
This skill supplements general code review guidance with project-specific priorities. The overriding principle
is **consistency with the existing codebase**. When reviewing code, look for places where new code diverges from
established patterns rather than evaluating it in isolation.

## Priority: Pattern Consistency

Before raising any issue, check whether the pattern in question is already established in the codebase. If existing
code follows the same approach as the code under review, it is not a review issue — even if a different approach might
be theoretically better. Consistency matters more than local optimality.
...
runs in task-runner mode. Review code should use:
...
- No secrets, API keys, or credentials in source code (detect-secrets should catch this, but review as a backstop)
