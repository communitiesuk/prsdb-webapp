...
open class JourneyStateSessionBuilder<SelfType : JourneyStateSessionBuilder<SelfType>>(
    ...
    fun withSelectedAddress(
        ...
        withCachedAddresses(listOf(AddressDataModel(singleLineAddress, localAuthorityId = localAuthority?.id, uprn = uprn)))
    ...
    fun withManualAddress(
        ...
        if (localAuthority != null) {
            whenever(mockLocalAuthorityService.retrieveLocalAuthorityById(localAuthority.id)).thenReturn(localAuthority)
            val selectLocalAuthorityFormModel =
                SelectLocalAuthorityFormModel().apply {
                    localAuthorityId = localAuthority.id
                }
            withSubmittedValue(RegisterPropertyStepId.LocalAuthority.urlPathSegment, selectLocalAuthorityFormModel)
        }
