...
class ParentageTests {
    ...
    fun `SingleParent allows child based on condition`() {
        val allowingParent = SingleParent(mock<JourneyStep.RequestableStep<TestEnum, *, JourneyState>>()) { true }
        val disallowingParent = SingleParent(mock<JourneyStep.RequestableStep<TestEnum, *, JourneyState>>()) { false }
    ...
    @Test
    fun `doesNotHaveOutcome returns a single parent with the condition checking that step's outcome is a different value`() {
        // Arrange
        val step = mock<JourneyStep.RequestableStep<TestEnum, *, *>>()
    ...
    fun `isComplete returns a single parent with the condition checking that tasks final step is complete`() {
        ...
        // Assert
        whenever(step.outcome).thenReturn(SubjourneyComplete.COMPLETE)
        assertTrue(parent.allowsChild())
