...
class EicrCyaSummaryRowsFactoryTests {
    ...
    private val expiredEicrIssueDate = LocalDate.now().minusYears((EICR_SAFETY_VALIDITY_YEARS + 1).toLong())
        ...
        val expectedRows =
            listOf(
                ...
                SummaryListRowViewModel.forCheckYourAnswersPage(
                    ...
                    validEicrIssueDate.toKotlinLocalDate().plus(DatePeriod(years = EICR_SAFETY_VALIDITY_YEARS)),
                    null,
                ),
            )
    ...
    fun `createRows returns correct rows when expired certificate has been provided`() {
        ...
        val expectedRows =
            listOf(
                ...
                SummaryListRowViewModel.forCheckYourAnswersPage(
                    "forms.checkComplianceAnswers.certificate.validUntil",
                    expiredEicrIssueDate.toKotlinLocalDate().plus(DatePeriod(years = EICR_SAFETY_VALIDITY_YEARS)),
                    null,
                ),
            )
