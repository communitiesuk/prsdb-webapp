...
class UpdateOrganisationMainContactControllerTests(
    ...
    private val updateOrgMainContactRoute =
    ...
    fun `getUpdateStep returns a redirect for unauthenticated user`() {
        mvc.get(updateOrgMainContactRoute).andExpect {
    ...
    fun `getUpdateStep returns 200 for an organisation landlord`() {
        ...
        ).thenReturn(mapOf(OrgMainContactStep.ROUTE_SEGMENT to mockStepLifecycleOrchestrator))
        whenever(
    ...
    fun `postUpdateStep returns a redirect for unauthenticated user`() {
        mvc
            .post(updateOrgMainContactRoute) {
                param("formData", "")
                with(csrf())
            }.andExpect {
                status { is3xxRedirection() }
