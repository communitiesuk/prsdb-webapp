...
class DeregisterCheckPendingInvitationsStepConfigTests {
    @Mock
    ...
    fun `mode returns null when form model is not present in state`() {
        ...
        whenever(mockState.getStepData(DeregisterCheckPendingInvitationsStep.ROUTE_SEGMENT)).thenReturn(null)
    ...
    fun `mode returns COMPLETE when form model is present in state`() {
        val stepConfig = setupStepConfig()
        whenever(mockState.getStepData(DeregisterCheckPendingInvitationsStep.ROUTE_SEGMENT)).thenReturn(emptyMap())
    ...
    private fun setupStepConfig(): DeregisterCheckPendingInvitationsStepConfig {
        val stepConfig = DeregisterCheckPendingInvitationsStepConfig(mockPropertyOwnershipService, mockJointLandlordInvitationService)
        stepConfig.routeSegment = DeregisterCheckPendingInvitationsStep.ROUTE_SEGMENT
