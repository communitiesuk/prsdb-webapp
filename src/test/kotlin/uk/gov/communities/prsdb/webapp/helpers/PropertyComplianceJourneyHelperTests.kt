...
class PropertyComplianceJourneyHelperTests {
    ...
    fun `getCertFilename with FileCategory delegates to getCertFilename with the expected step name`(
        ...
        val actualFileName = PropertyComplianceJourneyHelper.getCertFilename(PROPERTY_OWNERSHIP_ID, callbackType)
    ...
    companion object {
        ...
        private fun provideFileCategoryAndExpectedStepNames() =
            arrayOf(
                Arguments.of(
                    Named.of(CallbackType.GasSafetyCert.name, CallbackType.GasSafetyCert),
                    PropertyComplianceStepId.GasSafetyUpload.urlPathSegment,
                ),
                Arguments.of(
                    Named.of(CallbackType.Eicr.name, CallbackType.Eicr),
