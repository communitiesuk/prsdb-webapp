...
@WebMvcTest(LettingAgentUpdateFurnishedStatusController::class)
class LettingAgentUpdateFurnishedStatusControllerTests(
    ...
    private val formContent = "furnishedStatus=FURNISHED"
    ...
    fun `getUpdateStep seeds the journey with the token and returns to the letting agent property details page if journey not found`() {
        ...
        whenever(journeyFactory.initializeJourneyState(eq(token), eq(propertyOwnership.id))).thenReturn("journey-id")

        mvc.get(updateStepRoute).andExpect {
            status { is3xxRedirection() }
        }

        verify(journeyFactory).initializeJourneyState(eq(token), eq(propertyOwnership.id))
