...
object NftDataFaker {
    ...
    fun generatePropertyComplianceData(createdDateTimestamp: Timestamp): PropertyComplianceData {
        ...
        val eicrIssueDate =
            if (!hasEicrExemption && !eicrMissing) {
                generateDateBefore(createdDate, (ELECTRICAL_SAFETY_VALIDITY_YEARS * 365 * 1.5).toLong())
            } else {
                null
            }
    ...
    data class PropertyComplianceData(
        ...
        val electricalSafetyExpiryDate
            get() = eicrIssueDate?.let { Date.valueOf(it.toLocalDate().plusYears(ELECTRICAL_SAFETY_VALIDITY_YEARS.toLong())) }
    }
}
