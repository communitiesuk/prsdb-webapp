    ...
    fun preparePropertyOwnershipStatement(): PreparedStatement {
        val query =
            ...
            (id, created_date, last_modified_date, ownership_type, current_num_households, current_num_tenants, registration_number_id, 
             ...
             bills_included_list, custom_bills_included, furnished_status, rent_frequency, custom_rent_frequency, rent_amount, is_occupied, license_provide_later, tenancy_provide_later, is_active) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, true)
            """
        return connection.prepareStatement(query)
    }
