...
import uk.gov.communities.prsdb.webapp.journeys.propertyRegistration.states.EpcDetailsTaskDependencies
...
// An EPC-details task that owns its own steps and IS its own EpcState, so it can be composed into a journey via
// duplicableTask. Structure only: makeSubJourney defines the step flow. Instance-specific state (isOccupied, uprn,
// allowProvideCertificateLaterRoute) is supplied by the mount site via an EpcDetailsTaskDependencies contract.
...
class EpcDetailsTask(
...
) : DuplicableTaskWithDependencies<EpcState, EpcDetailsTaskDependencies>(journeyStateService),
    EpcState {
    ...
    override val uprn: Long? get() = dependencies.uprn
