...
@PrsdbController
@RequestMapping(LETTING_AGENT_UPDATE_RENT_FREQUENCY_AND_AMOUNT_ROUTE)
class LettingAgentUpdateRentFrequencyAndAmountController(
    ...
    companion object {
        const val LETTING_AGENT_UPDATE_RENT_FREQUENCY_AND_AMOUNT_ROUTE =
            "/$LANDLORD_PATH_SEGMENT/$LETTING_AGENT_PATH_SEGMENT/$PROPERTY_DETAILS_SEGMENT/{token}/update-rent-frequency-and-amount"

        fun getUpdateRentFrequencyAndAmountRoute(token: UUID): String =
            UriTemplate(LETTING_AGENT_UPDATE_RENT_FREQUENCY_AND_AMOUNT_ROUTE).expand(token).toASCIIString()
    }
}
