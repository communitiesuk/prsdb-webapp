...
@PrsdbController
@RequestMapping(LETTING_AGENT_UPDATE_EPC_ROUTE)
class LettingAgentUpdateEpcController(
    ...
    companion object {
        const val LETTING_AGENT_UPDATE_EPC_ROUTE =
            "/$LANDLORD_PATH_SEGMENT/$LETTING_AGENT_PATH_SEGMENT/$PROPERTY_DETAILS_SEGMENT/{token}/update-epc"

        fun getUpdateEpcRoute(token: UUID): String = UriTemplate(LETTING_AGENT_UPDATE_EPC_ROUTE).expand(token).toASCIIString()
    }
}
