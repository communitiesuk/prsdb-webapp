...
class EicrExemptionMissingStepConfig : AbstractRequestableStepConfig<Complete, NoInputFormModel, EicrState>() {
    ...
    override fun getStepSpecificContent(state: EicrState): Map<String, Any?> =
        mapOf(
            ...
            "submitButtonText" to if (state.isCheckingAnswers) "forms.buttons.saveAndContinue" else "forms.buttons.saveAndContinueToEPC",
        )

    override fun chooseTemplate(state: EicrState): String = "forms/eicrExemptionMissingForm"

    override fun mode(state: EicrState) = getFormModelFromStateOrNull(state)?.let { Complete.COMPLETE }
}

@JourneyFrameworkComponent
final class EicrExemptionMissingStep(
    stepConfig: EicrExemptionMissingStepConfig,
) : RequestableStep<Complete, NoInputFormModel, EicrState>(stepConfig) {
    companion object {
        const val ROUTE_SEGMENT = "eicr-exemption-missing"
    }
}
