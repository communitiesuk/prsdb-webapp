ALTER TABLE property_ownership
    ADD property_build_type SMALLINT,
    ADD address_id BIGINT REFERENCES address (id);

UPDATE property_ownership po SET (property_build_type, address_id) = (
    SELECT p.property_build_type, p.address_id FROM property p WHERE p.id = po.property_id
);

ALTER TABLE property_ownership
    ALTER COLUMN property_build_type SET NOT NULL,
