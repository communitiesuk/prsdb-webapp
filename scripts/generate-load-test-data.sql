...
BEGIN
    ...
    EXECUTE format(
        'INSERT INTO property_ownership (is_active, ownership_type, registration_number_id, primary_landlord_id, address_id, license_id, property_build_type) VALUES %s;',
    ...
    LOOP
        ...
        FOR propertyIndex IN 1..2 LOOP
            ...
            IF propertyIndex = 1 THEN
                ...
                propertyOwnerships := propertyOwnerships || '(' || concat_ws(',', 'true', 0, prnId, landlordId, unusedAddressIds[unusedAddressIdIndex], licenseId, 0) || '),';
                licenseId := licenseId + 1;
            ELSE
                propertyOwnerships := propertyOwnerships || '(' || concat_ws(',', 'true', 0, prnId, landlordId, unusedAddressIds[unusedAddressIdIndex], 'null', 0) || '),';
            END IF;
